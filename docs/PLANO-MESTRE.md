...
- [x] ✅ **Corrigir o ranking "O que estudar"** (dor do Pablo 2026-06-20: mostrava coisas que ele já tem + frases de requisito viradas "skill"). `estudo_gaps`: (1) skills saem só do `stack` (tags limpas) — frases de `requisitos_obrigatorios`/`desejaveis` viram só **evidência** pra marcar obrigatoriedade (busca por palavra inteira, `_word_in`/`_norm_text`); (2) casamento perfil×vaga robusto — `_formas_perfil` tira o parêntese descritivo e aproveita as tools de dentro dele (`(Playwright, BeautifulSoup)`), quebra compostos (`Docker / docker-compose`), e `_COBRE` mapeia sinônimos/derivações (PostgreSQL→SQL, TypeScript→JavaScript, "integração com LLMs"→Claude/Gemini/agentes, "engenharia de prompt"→prompt engineering); (3) `_norm_skill` strippa parênteses (dedup `IA (Inteligência Artificial)`→`IA`); (4) `_eh_skill` descarta tag-frase/soft-skill. Resultado: SQL/Git/APIs/IA/Claude/LLMs/JS/Docker saíram dos gaps (viraram pontos fortes), lixo de frase sumiu. *(sem migração; só agregação.)*
