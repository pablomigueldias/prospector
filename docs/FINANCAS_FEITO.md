...
- ✅ **/resumo por período** (2026-06-13) — `/resumo julho`, `/resumo 07`, `/resumo 07/2025`, `/resumo 2025-07` (mês nominal/abreviado/número/MM-AAAA/AAAA-MM); mês nominal futuro cai na ocorrência mais recente (ano anterior). Sem arg = mês atual. `bot_service._parse_periodo`. Smoke `tests/test_financas_bot_resumo_periodo_api.py`.
- ✅ **Lançar despesa prevista/agendada** (2026-06-13) — falar *"vou pagar 200 de luz dia 10"* (ou qualquer frase com marcador de futuro / data à frente) faz o card nascer como **prevista**: ao confirmar, status `prevista` + `data_vencimento`, **não mexe no saldo** e aparece em "A pagar". Heurística `bot_service._eh_prevista` (sem mudar o prompt do Gemini). `DespesaCreate` ganhou `data_vencimento`. Smoke `tests/test_financas_bot_prevista_api.py`.
...
- ✅ **Projeção de fim de mês (sobra estimada)** (2026-06-13) — card no topo do dashboard: **sobra estimada = saldo atual + a receber − a pagar** (previstas/atrasadas com vencimento até o fim do mês, incl. vencidas). Verde/vermelho + aviso quando negativa. `GET /api/financas/resumo/projecao` → `resumo_service.projecao_mes` (repo `previstas_por_tipo`). Smoke `tests/test_financas_projecao_mes_api.py`. Ref `FinancasScreen.tsx`.
- ✅ **Alerta de saldo do mês negativo no digest** (2026-06-13) — quando a projeção fica no vermelho, o digest do Telegram abre com **🚨 Saldo do mês no vermelho** (sobra estimada + a decomposição). Reusa `projecao_mes`. `lembretes.py::_texto_saldo_negativo`. Smoke `test_financas_lembretes`.
- ✅ **Roll-up de orçamento por categoria-mãe** (2026-06-13) — o teto de uma categoria com subcategorias agora **soma os gastos das filhas** (antes era match exato). `orcamento_service.status_do_mes` percorre a árvore (`CategoriaRepository.listar_todas`). Smoke `tests/test_financas_orcamento_rollup_api.py`.
- ✅ **Reservas com objetivo** (2026-06-13) — conta tipo *reserva* ganhou **meta** (ex.: "viagem: 5.000"): o card mostra uma **barra de progresso** (% do objetivo, 🎉 ao atingir). Migration `a7c3e1f9d2b8` (`contas.meta`). `meta` no CRUD de conta + campo no form (aparece pra reserva). Smoke `tests/test_financas_conta_meta_api.py`. Ref `ContasSection.tsx`.
