...
- ✅ **Juros/multa de boleto vencido** (2026-06-13) — a IA lê os encargos do boleto (multa % + juros de mora % a.m.); o painel A pagar mostra os vencidos já com os juros acumulados até hoje, e o modal de pagamento desdobra valor + multa + juros (N dias) = total (recalcula pela data). Ao pagar, o saldo desce pelo total e o que foi de encargos fica em `encargos_pagos`. `encargos.py` (back) / `lib/encargos.ts` (front). Migration `a1c7e9d2b4f0`.
