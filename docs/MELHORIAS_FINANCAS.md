...
- 🔴 **Telas de CRUD que faltam no front** — criar/editar **conta**, **categoria**, **cartão**, **recorrência** e **transação** pela interface (boa parte hoje só existe via API/bot).
- 🟡 **Lista de transações filtrável** — por mês, conta, categoria, tipo, com busca e edição inline.
