...
- 🟡 **Registrar leitura de consumo pela web** — `POST /api/financas/leituras`: a `ConsumoSection` só **lê**; não dá pra lançar a leitura de água/gás/luz manualmente (hoje só chega via boleto de condomínio).
